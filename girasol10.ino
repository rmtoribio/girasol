...
void loop() {
  ...
  ReadCompass();
  Serial.println(F("Leyendo sextante..."));
  ReadSextant();
  wdt_reset(); 
  ...
  case sNormal :
    ...
    Serial.println(F("calculando Azimut  elevacion...")); 
    azimuth  = calcAzimuth();
    elevation = calcElevation();

    Serial.println(F("manejando motor azimut..."));
    if      ( compass <  azimuth )             { moveAzimutMotor (forward); }
    else if ( compass > (azimuth + deltaA) )   { moveAzimutMotor (backward);}
    else                                       { moveAzimutMotor (stoped);  }
    
    Serial.println(F("manejando motor inclinacion..."));
  ...
  case sExecute :
     Serial.println(F("Ejecutando comamdo..."));
